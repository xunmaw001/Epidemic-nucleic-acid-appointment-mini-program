<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="检测编号" prop="jiancebianhao">
            <el-input v-model="ruleForm.jiancebianhao" 
                placeholder="检测编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.jiancebianhao" label="检测编号" prop="jiancebianhao">
              <el-input v-model="ruleForm.jiancebianhao" 
                placeholder="检测编号" readonly></el-input>
          </el-form-item>
        </div>
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="检测结果" prop="jiancejieguo">
          <el-select :disabled="ro.jiancejieguo" v-model="ruleForm.jiancejieguo" placeholder="请选择检测结果">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="检测结果" prop="jiancejieguo">
	      <el-input v-model="ruleForm.jiancejieguo"
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiancexinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiancexinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
